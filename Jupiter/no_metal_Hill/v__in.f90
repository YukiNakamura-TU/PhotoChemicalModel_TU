...
contains
  ...
  subroutine v__in__ini(spl, set) ! out
    ...
    ! directory setting
    set%dir_name = './Jupiter/no_metal_Hill'
    set%fnamestable = './Jupiter/no_metal_Hill/output/density/n_stable.dat'
  ...
  subroutine v__in__exe(cst,      & ! in
    ...
    ! read P, L, J list
    open(11, file = './Jupiter/no_metal_Hill/input/PLJ_list/Production_list.dat', status = 'unknown' )
      do isp = 1, spl%nsp_i
        read(11,*) (spl%Prod_list(isp,ich), ich = 1, spl%nch_P)
      end do
    close(11)

    open(11, file = './Jupiter/no_metal_Hill/input/PLJ_list/Loss_list.dat', status = 'unknown' )
      do isp = 1, spl%nsp_i
        read(11,*) (spl%Loss_list(isp,ich), ich = 1, spl%nch_L)
      end do
    close(11)

    open(11, file = './Jupiter/no_metal_Hill/input/PLJ_list/Jacobian_list.dat', status = 'unknown' )
      do i = 1, spl%n_Jlist
        read(11,*) (spl%Jmtx_list(i,j), j = 1, spl%nch_J)
      end do
    close(11)

    open(11, file = './Jupiter/no_metal_Hill/input/PLJ_list/reactant_list.dat', status = 'unknown' )
      do ich = 1, spl%nch
        read(11,*) (spl%reactant_list(ich,isp), isp = 1, 10)
      end do
    close(11)

    open(11, file = './Jupiter/no_metal_Hill/input/PLJ_list/product_list.dat', status = 'unknown' )
      do ich = 1, spl%nch
        read(11,*) (spl%product_list(ich,isp), isp = 1, 10)
      end do
    close(11)

    open(11, file = './Jupiter/no_metal_Hill/input/PLJ_list/reaction_type_list.dat', status = 'unknown' )
      do ich = 1, spl%nch
        read(11,*) spl%reaction_type_list(ich)
      end do
    close(11)

    open(11, file = './Jupiter/no_metal_Hill/input/PLJ_list/rate_rpn_token.dat', status = 'unknown' )
      do ich = 1, spl%nch
        do i = 1, 3
    ...
    open(11, file = './Jupiter/no_metal_Hill/input/PLJ_list/rate_rpn_label.dat', status = 'unknown' )
      do ich = 1, spl%nch
        do i = 1, 3
    ...
    open(11, file = './Jupiter/no_metal_Hill/input/PLJ_list/rate_cases.dat', status = 'unknown' )
      read(11,*) (spl%rate_cases(ich), ich = 1, spl%nch)
    close(11)

    open(11, file = './Jupiter/no_metal_Hill/input/PLJ_list/T_range.dat', status = 'unknown' )
      do ich = 1, spl%nch
        do i = 1, 3
    ...
    open(11, file = './Jupiter/no_metal_Hill/input/Temperature/T_e.dat', status = 'unknown' )
      do iz = 1, grd%nz
        read(11,*) tmp,var%Te(iz)
      end do
    close(11)

    open(11, file = './Jupiter/no_metal_Hill/input/Temperature/T_i.dat', status = 'unknown' )
      do iz = 1, grd%nz
        read(11,*) tmp,var%Ti(iz)
      end do
    close(11)

    open(11, file = './Jupiter/no_metal_Hill/input/Temperature/T_n.dat', status = 'unknown' )
      do iz = 1, grd%nz
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Jupiter/no_metal_Hill/input/density/H2.dat', status = 'unknown' )
        do iz = 1, grd%nz
    ...
    isp = sp_index(spl, 'He')
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Jupiter/no_metal_Hill/input/density/He.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Jupiter/no_metal_Hill/input/density/CH4.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Jupiter/no_metal_Hill/input/density/C2H2.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Jupiter/no_metal_Hill/input/density/C2H4.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Jupiter/no_metal_Hill/input/density/C2H6.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Jupiter/no_metal_Hill/input/density/H2_v2.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Jupiter/no_metal_Hill/input/density/H2_v4.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
