...
contains
  ...
  subroutine v__in__ini(spl, set) ! out
    ...
    ! directory setting
    set%dir_name = './Jupiter/metal_dev10_Hill'
    set%fnamestable = './Jupiter/metal_dev10_Hill/output/density/n_stable.dat'
  ...
  subroutine v__in__exe(cst,      & ! in
    ...
    var%m(67) = 56.85294000_dp * cst%m_u !FeH
    var%m(68) = 56.85239142_dp * cst%m_u !FeH+
    var%m(69) = 57.86033142_dp * cst%m_u !FeH2+
    var%m(70) = 71.88691142_dp * cst%m_u !FeCH4+
    var%m(71) = 81.88173142_dp * cst%m_u !FeC2H2+
    var%m(72) = 83.89761142_dp * cst%m_u !FeC2H4+
    var%m(73) = 25.31294000_dp * cst%m_u !MgH
    var%m(74) = 25.31239142_dp * cst%m_u !MgH+
    var%m(75) = 26.32033142_dp * cst%m_u !MgH2+
    var%m(76) = 40.34691142_dp * cst%m_u !MgCH4+
    var%m(77) = 50.34173142_dp * cst%m_u !MgC2H2+
    var%m(78) = 52.35761142_dp * cst%m_u !MgC2H4+
    var%m(79) = 29.09344000_dp * cst%m_u !SiH
    var%m(80) = 29.09289142_dp * cst%m_u !SiH+
    var%m(81) = 30.10083142_dp * cst%m_u !SiH2+
    var%m(82) = 28.08495142_dp * cst%m_u !SiCnHm+
    var%m(83) = 41.10414000_dp * cst%m_u !SiCH
    var%m(84) = 42.11208000_dp * cst%m_u !SiCH2
    ...
    ! read P, L, J list
    open(11, file = './Jupiter/metal_dev10_Hill/input/PLJ_list/Production_list.dat', status = 'unknown' )
      do isp = 1, spl%nsp_i
        read(11,*) (spl%Prod_list(isp,ich), ich = 1, spl%nch_P)
      end do
    close(11)

    open(11, file = './Jupiter/metal_dev10_Hill/input/PLJ_list/Loss_list.dat', status = 'unknown' )
      do isp = 1, spl%nsp_i
        read(11,*) (spl%Loss_list(isp,ich), ich = 1, spl%nch_L)
      end do
    close(11)

    open(11, file = './Jupiter/metal_dev10_Hill/input/PLJ_list/Jacobian_list.dat', status = 'unknown' )
      do i = 1, spl%n_Jlist
        read(11,*) (spl%Jmtx_list(i,j), j = 1, spl%nch_J)
      end do
    close(11)

    open(11, file = './Jupiter/metal_dev10_Hill/input/PLJ_list/reactant_list.dat', status = 'unknown' )
      do ich = 1, spl%nch
        read(11,*) (spl%reactant_list(ich,isp), isp = 1, 10)
      end do
    close(11)

    open(11, file = './Jupiter/metal_dev10_Hill/input/PLJ_list/product_list.dat', status = 'unknown' )
      do ich = 1, spl%nch
        read(11,*) (spl%product_list(ich,isp), isp = 1, 10)
      end do
    close(11)

    open(11, file = './Jupiter/metal_dev10_Hill/input/PLJ_list/reaction_type_list.dat', status = 'unknown' )
      do ich = 1, spl%nch
        read(11,*) spl%reaction_type_list(ich)
      end do
    close(11)

    open(11, file = './Jupiter/metal_dev10_Hill/input/PLJ_list/rate_rpn_token.dat', status = 'unknown' )
      do ich = 1, spl%nch
        do i = 1, 3
    ...
    open(11, file = './Jupiter/metal_dev10_Hill/input/PLJ_list/rate_rpn_label.dat', status = 'unknown' )
      do ich = 1, spl%nch
        do i = 1, 3
    ...
    open(11, file = './Jupiter/metal_dev10_Hill/input/PLJ_list/rate_cases.dat', status = 'unknown' )
      read(11,*) (spl%rate_cases(ich), ich = 1, spl%nch)
    close(11)

    open(11, file = './Jupiter/metal_dev10_Hill/input/PLJ_list/T_range.dat', status = 'unknown' )
      do ich = 1, spl%nch
        do i = 1, 3
    ...
    open(11, file = './Jupiter/metal_dev10_Hill/input/Temperature/T_e.dat', status = 'unknown' )
      do iz = 1, grd%nz
        read(11,*) tmp,var%Te(iz)
      end do
    close(11)

    open(11, file = './Jupiter/metal_dev10_Hill/input/Temperature/T_i.dat', status = 'unknown' )
      do iz = 1, grd%nz
        read(11,*) tmp,var%Ti(iz)
      end do
    close(11)

    open(11, file = './Jupiter/metal_dev10_Hill/input/Temperature/T_n.dat', status = 'unknown' )
      do iz = 1, grd%nz
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Jupiter/metal_dev10_Hill/input/density/H2.dat', status = 'unknown' )
        do iz = 1, grd%nz
    ...
    isp = sp_index(spl, 'He')
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Jupiter/metal_dev10_Hill/input/density/He.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Jupiter/metal_dev10_Hill/input/density/CH4.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Jupiter/metal_dev10_Hill/input/density/C2H2.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Jupiter/metal_dev10_Hill/input/density/C2H4.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Jupiter/metal_dev10_Hill/input/density/C2H6.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Jupiter/metal_dev10_Hill/input/density/H2_v2.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Jupiter/metal_dev10_Hill/input/density/H2_v4.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
