...
contains
  ...
  !-------------------------------------------------
  subroutine p__EUVAC_flux(spl, grd,     & ! in
    &                      var, xct, flx ) ! inout
    ...
    flx%F10_7 = 140.0_dp ! F10.7 at Earth
    ...
    do iwl = 1, flx%nwl_EUV
      flx%solar_EUV(iwl) = flx%F74113(iwl) * (1.0_dp + flx%Ai(iwl) * (flx%F10_7 - 80.0_dp)) * 1.0e+13_dp &![m^-2 s^-1]
    ...
    subroutine p__EUVAC_cross_section(spl, & ! in
      ...
      ! 化学種検索を改善予定；p__search使用すると遅くなる？微々たるさなら実装
        ...
        do ich = 1, spl%nch
          if (spl%reaction_type_char(ich) == 'photoionization') then
            ...
            & .and. products(1) == 'O2+' .and. products(2) == 'e-' ) then
              ...
              xct%sigma_i_EUV(36,ich) =  0.0d-22
