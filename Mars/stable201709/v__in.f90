...
contains
  ...
  subroutine v__in__ini(spl, set) ! out
    ...
    ! directory setting
    set%dir_name = './Mars/stable201709'
    set%fnamestable = './Mars/stable201709/output/density/n_stable.dat'
  ...
  subroutine v__in__exe(cst,      & ! in
    ...
    var%m(4) = 15.99940000_dp * cst%m_u !O(1D)
    var%m(5) = 18.01528000_dp * cst%m_u !H2O
    var%m(6) = 1.00794000_dp * cst%m_u !H
    var%m(7) = 17.00734000_dp * cst%m_u !OH
    var%m(8) = 2.01588000_dp * cst%m_u !H2
    var%m(9) = 47.99820000_dp * cst%m_u !O3
    var%m(10) = 31.99880000_dp * cst%m_u !O2
    ...
    ! read P, L, J list
    open(11, file = './Mars/stable201709/input/PLJ_list/Production_list.dat', status = 'unknown' )
      do isp = 1, spl%nsp_i
        read(11,*) (spl%Prod_list(isp,ich), ich = 1, spl%nch_P)
      end do
    close(11)

    open(11, file = './Mars/stable201709/input/PLJ_list/Loss_list.dat', status = 'unknown' )
      do isp = 1, spl%nsp_i
        read(11,*) (spl%Loss_list(isp,ich), ich = 1, spl%nch_L)
      end do
    close(11)

    open(11, file = './Mars/stable201709/input/PLJ_list/Jacobian_list.dat', status = 'unknown' )
      do i = 1, spl%n_Jlist
        read(11,*) (spl%Jmtx_list(i,j), j = 1, spl%nch_J)
      end do
    close(11)

    open(11, file = './Mars/stable201709/input/PLJ_list/reactant_list.dat', status = 'unknown' )
      do ich = 1, spl%nch
        read(11,*) (spl%reactant_list(ich,isp), isp = 1, 10)
      end do
    close(11)

    open(11, file = './Mars/stable201709/input/PLJ_list/product_list.dat', status = 'unknown' )
      do ich = 1, spl%nch
        read(11,*) (spl%product_list(ich,isp), isp = 1, 10)
      end do
    close(11)

    open(11, file = './Mars/stable201709/input/PLJ_list/reaction_type_list.dat', status = 'unknown' )
      do ich = 1, spl%nch
        read(11,*) spl%reaction_type_list(ich)
      end do
    close(11)

    open(11, file = './Mars/stable201709/input/PLJ_list/rate_rpn_token.dat', status = 'unknown' )
      do ich = 1, spl%nch
        do i = 1, 3
    ...
    open(11, file = './Mars/stable201709/input/PLJ_list/rate_rpn_label.dat', status = 'unknown' )
      do ich = 1, spl%nch
        do i = 1, 3
    ...
    open(11, file = './Mars/stable201709/input/PLJ_list/rate_cases.dat', status = 'unknown' )
      read(11,*) (spl%rate_cases(ich), ich = 1, spl%nch)
    close(11)

    open(11, file = './Mars/stable201709/input/PLJ_list/T_range.dat', status = 'unknown' )
      do ich = 1, spl%nch
        do i = 1, 3
    ...
    open(11, file = './Mars/stable201709/input/Temperature/T_e.dat', status = 'unknown' )
      do iz = 1, grd%nz
        read(11,*) tmp,var%Te(iz)
      end do
    close(11)

    open(11, file = './Mars/stable201709/input/Temperature/T_i.dat', status = 'unknown' )
      do iz = 1, grd%nz
        read(11,*) tmp,var%Ti(iz)
      end do
    close(11)

    open(11, file = './Mars/stable201709/input/Temperature/T_n.dat', status = 'unknown' )
      do iz = 1, grd%nz
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Mars/stable201709/input/density/mcd_CO2_201709.txt', status = 'unknown' )
        do iz = 1, grd%nz
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Mars/stable201709/input/density/mcd_H2O_201709.txt', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    isp = sp_index(spl, 'CO2+')
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Mars/stable201709/input/density/CO2+.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
