...
contains
  ...
  subroutine v__in__ini(spl, set) ! out
    ...
    set%dtime_limit = 1.0e0_dp
    set%latitude = 44.0_dp
    set%Ls = 0.0_dp
    set%nday = 3.0_dp
    set%scheme = 'implicit'
    set%inversion = 'Catling'
    ! directory setting
    set%dir_name = './Mars/Shinagawa'
    set%fnamestable = './Mars/Shinagawa/output/density/n_stable.dat'
  ...
  subroutine v__in__exe(cst,      & ! in
    ...
    grd%dalt(1:26) =  4.0e3_dp ! [m]
    grd%alt(1)      = 100.0e3_dp ! [m]
    do iz = 2, grd%nz
    ...
    spl%label_fix(6) = 1 ! O2: fixed
    spl%label_fix(7) = 0 ! O2+: variable
    spl%label_fix(8) = 1 ! O: fixed
    spl%label_fix(9) = 1 ! H: fixed
    spl%label_fix(10) = 0 ! H+: variable
    spl%label_fix(11) = 1 ! N2: fixed
    spl%label_fix(12) = 1 ! NO+: fixed
    spl%label_fix(13) = 1 ! N: fixed
    spl%label_fix(14) = 1 ! HCO+: fixed
    ...
    spl%all_to_var(7) = 4 ! O2+: variable
    spl%all_to_var(10) = 5 ! H+: variable

    ! var_to_all
    spl%var_to_all(1) = 2 ! CO2+: variable
    spl%var_to_all(2) = 3 ! e-: variable
    spl%var_to_all(3) = 4 ! O+(4S): variable
    spl%var_to_all(4) = 7 ! O2+: variable
    spl%var_to_all(5) = 10 ! H+: variable
    ...
    var%m(6) = 31.99880000_dp * cst%m_u !O2
    var%m(7) = 31.99825142_dp * cst%m_u !O2+
    var%m(8) = 15.99940000_dp * cst%m_u !O
    var%m(9) = 1.00794000_dp * cst%m_u !H
    var%m(10) = 1.00739142_dp * cst%m_u !H+
    var%m(11) = 28.01340000_dp * cst%m_u !N2
    var%m(12) = 30.00555142_dp * cst%m_u !NO+
    var%m(13) = 14.00670000_dp * cst%m_u !N
    var%m(14) = 29.01749142_dp * cst%m_u !HCO+
    ...
    var%q(6) = 0.0_dp * cst%q_e !O2
    var%q(7) = 1.0_dp * cst%q_e !O2+
    var%q(8) = 0.0_dp * cst%q_e !O
    var%q(9) = 0.0_dp * cst%q_e !H
    var%q(10) = 1.0_dp * cst%q_e !H+
    var%q(11) = 0.0_dp * cst%q_e !N2
    var%q(12) = 1.0_dp * cst%q_e !NO+
    var%q(13) = 0.0_dp * cst%q_e !N
    var%q(14) = 1.0_dp * cst%q_e !HCO+

    ! read P, L, J list
    open(11, file = './Mars/Shinagawa/input/PLJ_list/Production_list.dat', status = 'unknown' )
      do isp = 1, spl%nsp_i
        read(11,*) (spl%Prod_list(isp,ich), ich = 1, spl%nch_P)
      end do
    close(11)

    open(11, file = './Mars/Shinagawa/input/PLJ_list/Loss_list.dat', status = 'unknown' )
      do isp = 1, spl%nsp_i
        read(11,*) (spl%Loss_list(isp,ich), ich = 1, spl%nch_L)
      end do
    close(11)

    open(11, file = './Mars/Shinagawa/input/PLJ_list/Jacobian_list.dat', status = 'unknown' )
      do i = 1, spl%n_Jlist
        read(11,*) (spl%Jmtx_list(i,j), j = 1, spl%nch_J)
      end do
    close(11)

    open(11, file = './Mars/Shinagawa/input/PLJ_list/reactant_list.dat', status = 'unknown' )
      do ich = 1, spl%nch
        read(11,*) (spl%reactant_list(ich,isp), isp = 1, 10)
      end do
    close(11)

    open(11, file = './Mars/Shinagawa/input/PLJ_list/product_list.dat', status = 'unknown' )
      do ich = 1, spl%nch
        read(11,*) (spl%product_list(ich,isp), isp = 1, 10)
      end do
    close(11)

    open(11, file = './Mars/Shinagawa/input/PLJ_list/reaction_type_list.dat', status = 'unknown' )
      do ich = 1, spl%nch
        read(11,*) spl%reaction_type_list(ich)
      end do
    close(11)

    open(11, file = './Mars/Shinagawa/input/PLJ_list/rate_rpn_token.dat', status = 'unknown' )
      do ich = 1, spl%nch
        do i = 1, 3
    ...
    open(11, file = './Mars/Shinagawa/input/PLJ_list/rate_rpn_label.dat', status = 'unknown' )
      do ich = 1, spl%nch
        do i = 1, 3
    ...
    open(11, file = './Mars/Shinagawa/input/PLJ_list/rate_cases.dat', status = 'unknown' )
      read(11,*) (spl%rate_cases(ich), ich = 1, spl%nch)
    close(11)

    open(11, file = './Mars/Shinagawa/input/PLJ_list/T_range.dat', status = 'unknown' )
      do ich = 1, spl%nch
        do i = 1, 3
    ...
    open(11, file = './Mars/Shinagawa/input/Temperature/T_e.dat', status = 'unknown' )
      do iz = 1, grd%nz
        read(11,*) tmp,var%Te(iz)
      end do
    close(11)

    open(11, file = './Mars/Shinagawa/input/Temperature/T_i.dat', status = 'unknown' )
      do iz = 1, grd%nz
        read(11,*) tmp,var%Ti(iz)
      end do
    close(11)

    open(11, file = './Mars/Shinagawa/input/Temperature/T_n.dat', status = 'unknown' )
      do iz = 1, grd%nz
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Mars/Shinagawa/input/density/CO2.dat', status = 'unknown' )
        do iz = 1, grd%nz
    ...
    isp = sp_index(spl, 'CO')
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Mars/Shinagawa/input/density/CO.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    isp = sp_index(spl, 'O2')
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Mars/Shinagawa/input/density/O2.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    isp = sp_index(spl, 'O')
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Mars/Shinagawa/input/density/O.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    isp = sp_index(spl, 'H')
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Mars/Shinagawa/input/density/H.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    isp = sp_index(spl, 'N2')
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Mars/Shinagawa/input/density/N2.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    isp = sp_index(spl, 'NO+')
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Mars/Shinagawa/input/density/NOi.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    isp = sp_index(spl, 'N')
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Mars/Shinagawa/input/density/N.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
    ...
    if (isp >= 1 .and. isp <= spl%nsp) then
      open(11, file = './Mars/Shinagawa/input/density/HCOi.dat', status = 'unknown' )
        do iz = 1, grd%nz
          read(11,*) tmp, var%ni(isp,iz)
        end do
      close(11)
