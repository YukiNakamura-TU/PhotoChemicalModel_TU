...
#This Julia code is modified to regenerate the result of Zanhle+2008 by Shungo Koyama
#Krasnopolsky+2010 did not confirm the result so this time I'll check this again.
#It's related to CO2 instability problem on Mars.2019/4/1

#Then I included deposition flux, temperature profile change,,,,
#To investigate Self-regulation system
...
#I deleted HOCO by changing HOCO-involved reactions' coefficient to zero to run model in a condition of more CO2 and H2O
...
function plotDiff(n_current)
    figure()
    plot(map(z->Keddy(n_current,z),alt[1:end-1]),alt[1:end-1])
    plot(map(z->Dcoef(z,:H, n_current),alt[1:end-1]),alt[1:end-1])
    plot(map(z->Dcoef(z,:H2, n_current),alt[1:end-1]),alt[1:end-1])
    plot(map(z->Dcoef(z,:O, n_current),alt[1:end-1]),alt[1:end-1])
    xlim(1e5,1e10)
    xscale("log")
...
for i in [round(Int, floor(minimum(o3chapxdata[:,1]))):round(Int, ceil(maximum(o3chapxdata[:,1]))-1);]
