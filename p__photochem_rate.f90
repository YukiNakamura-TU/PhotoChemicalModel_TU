...
contains
  ...
  subroutine p__photochem_rate__exe(spl, cst, flx, grd, set, & ! in
    ...
    integer isp, jsp, ksp, ich, jch, iz, iwl, icase
    ...
    ! reaction rate -------------------------------------------------------------------------------
      ...
      do ich = 1, spl%nch
        if (spl%reaction_type_char(ich) == 'photodissociation') then
          do iwl = 1, flx%nwl_UV
            do iz = 1, grd%nz
              var%ki(ich,iz) = var%ki(ich,iz) &
                &             + var%I_UV(iwl,iz) * xct%sigma_d_UV(iwl,iz,ich)
            end do
          end do
        end if
      end do
        ...
        do ich = 1, spl%nch
          ...
          if (spl%reaction_type_list(ich) == 0) then
              do icase = 1, spl%rate_cases(ich)
                ...
                if (      spl%T_range(ich,icase,1) > 0_dp - 0.1_dp &
                  & .and. spl%T_range(ich,icase,1) < 0_dp + 0.1_dp) then
                  T = var%Tn(iz)
                end if
                if (      spl%T_range(ich,icase,1) > 1_dp - 0.1_dp &
                  & .and. spl%T_range(ich,icase,1) < 1_dp + 0.1_dp) then
                  T = var%Tn(iz)
                end if
                if (      spl%T_range(ich,icase,1) > 2_dp - 0.1_dp &
                  & .and. spl%T_range(ich,icase,1) < 2_dp + 0.1_dp) then
                  T = var%Te(iz)
                end if
                if (      spl%T_range(ich,icase,1) > 3_dp - 0.1_dp &
                  & .and. spl%T_range(ich,icase,1) < 3_dp + 0.1_dp) then
                  T = var%Ti(iz)
                end if
      ...
      do iz = 1, grd%nz
        ...
        do jch = 1, var%nspecial
          ...
          &  .or. spl%reaction_type_char(ich) == 'Meteoroid ablation' ) then
